      ...
      const data = await axios({
        ...
        if ((errorResponse.status === 400) && ((errorResponse.data.retMessage === "9") || (errorResponse.data.retMessage === 9) )) { // errorResponse.statusText === "Bad Request" && 
          ...
          if (errorResponse.data.retObject.time){
            time = ', el tiempo para poder cancelar es de ' + errorResponse.data.retObject.time + (errorResponse.data.retObject.time>=1?' horas':'hora') + ' antes del comienzo de la reserva.' ;
          }
          ...
          this.request.text.body = 'Lo sentimos, no se puede realizar esta accion' + time;
        }
