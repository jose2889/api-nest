...
export class GoogleAuthService {
  ...
  async getAccessTokenFromRefreshToken(refreshToken: string): Promise<string> {
    const { tokens } = await this.googleOAuth2Client.refreshToken(refreshToken);
    return tokens.access_token;
  }

  async getGoogleCalendarRequestHeaders(accessToken: string): Promise<any> {
    const headers = {
      Authorization: `Bearer ${accessToken}`,
    };
    return this.googleOAuth2Client.getRequestHeaders(headers);
  }
}
